...
void Display3() {
    double ratia = 0.05;
    double xmax = 100 + ratia, ymax= 1 + ratia;
    double x, y;
    glColor3f(1, 0.1, 0.1); // rosu
    glBegin(GL_LINE_STRIP);
    for (x = 0; x <= 100; x += ratia) {
        ...
        {
            y = (fabs(ceil(x) - x) <= fabs(floor(x) - x)) ? fabs(ceil(x) - x) / x : fabs(floor(x) - x) / x;
        }
...
void Display4() {
    double ratia = 0.014, a = 0.2;
    double pi = 4 * atan(1.0);
    double x, y, xant, yant, count = 0;
    int red = 1;
    glColor3f(1, 0.1, 0.1);
    glBegin(GL_TRIANGLES);
    xant = a / (4 * pow(cos(-pi / 2 + ratia), 2) - 3);
    yant = a * tan(-pi / 2 + ratia) / (4 * pow(cos(-pi / 2 + ratia), 2) - 3);
    for (double t = -pi / 2 + 2 * ratia; t < pi / 2; t += ratia)
    {
        if (t != (pi / 6) && t != -(pi / 6))
        {
			x = a / (4 * pow(cos(t), 2) - 3);
...
void Display6() {
    double ratia = 0.003, a = 0.4;
    double pi = 4 * atan(1.0);
    double r, x, y, xprim, yprim, xultim, yultim;
    double t = -pi / 4 + ratia;
    glColor3f(1, 0.1, 0.1); // rosu'
    // scalare ? 
    glBegin(GL_LINE_STRIP);
